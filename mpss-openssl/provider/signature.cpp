...
namespace {
    ...
    extern "C" const OSSL_PARAM* mpss_signature_gettable_ctx_params(void* ctx, [[maybe_unused]] void* provctx) {
        ...
        static OSSL_PARAM ret_key_available[] = {
            OSSL_PARAM_octet_string(OSSL_SIGNATURE_PARAM_ALGORITHM_ID, nullptr, 0),
            OSSL_PARAM_END
        };

        static OSSL_PARAM ret_key_unavailable[] = {
            OSSL_PARAM_END
        };
    ...
    extern "C" int mpss_signature_get_ctx_params(void* ctx, OSSL_PARAM params[]) {
        ...
        if (!obj) {
            std::cout << "LOG: mpss_signature_get_ctx_params (failed to create ASN1_OBJECT from " << sctx->pkey->alg_name->c_str() << ")" << std::endl;
            return 0;
        }
    ...
    extern "C" int mpss_signature_sign(void* ctx, unsigned char* sig, ::size_t* siglen, ::size_t sigsize,
        ...
        std::transform(der_sig.begin(), der_sig.begin() + bytes_written, sig,
    ...
    extern "C" int mpss_signature_verify(void* ctx, const unsigned char* sig, ::size_t siglen, const unsigned char* tbs, ::size_t tbslen) {
        mpss_signature_ctx* sctx = static_cast<mpss_signature_ctx*>(ctx);
        if (!sctx || !sctx->pkey || !sctx->pkey->has_valid_key()) {
            return 0;
        }
    ...
    extern "C" int mpss_signature_digest_sign_init(void* ctx, const char* mdname, void* provkey, [[maybe_unused]] const OSSL_PARAM params[]) {
        mpss_signature_ctx* sctx = static_cast<mpss_signature_ctx*>(ctx);
        if (!sctx) {
            return 0;
        }
        ...
        if (!are_same_hash(std::string_view(mdname), hash_name)) {
            std::cout << "LOG: mpss_signature_digest_sign_init (mdname " << mdname << " is not supported) -> 0" << std::endl;
            return 0;
        }
    ...
    extern "C" int mpss_signature_digest_sign(void* ctx, unsigned char* sig, ::size_t* siglen, ::size_t sigsize, const unsigned char* tbs, ::size_t tbslen) {
        mpss_signature_ctx* sctx = static_cast<mpss_signature_ctx*>(ctx);
        if (!sctx) {
            return 0;
        }
    ...
    extern "C" int mpss_signature_digest_verify_init(void* ctx, const char* mdname, void* provkey, const OSSL_PARAM params[]) {
    ...
    extern "C" int mpss_signature_digest_verify(void* ctx, const unsigned char* sig, ::size_t siglen, const unsigned char* tbs, ::size_t tbslen) {
