...
#define MPSS_MAKE_DIGEST_NEWCTX(digest) \
    ...
    extern "C" int mpss_digest_final(void* ctx, unsigned char* out, ::size_t* outl, ::size_t outsz) {
        ...
        std::transform(dctx->digest.begin(), dctx->digest.end(), out, [](std::byte b) { return static_cast<unsigned char>(b); });
    ...
    int mpss_digest_digest_internal(void* ctx, const unsigned char* in, ::size_t inl, unsigned char* out, ::size_t* outl, ::size_t outsz) {
        if (1 != mpss_digest_init(ctx, nullptr)
            || (1 != mpss_digest_update(ctx, in, inl))
...
namespace mpss_openssl::provider {
    const OSSL_ALGORITHM mpss_digest_algorithms[] = {
